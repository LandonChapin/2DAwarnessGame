...
private:
	...
	std::vector<s_button> mainMenuButtonArray{
		new s_button{std::string("NEW GAME"), sf::Text(), Hitbox()},
		new s_button{std::string("LOAD GAME"), sf::Text(), Hitbox()},
		new s_button{std::string("SETTINGS"), sf::Text(), Hitbox()},
		new s_button{std::string("QUIT"), sf::Text(), Hitbox()}
	};
